...
If you're totally new to Ruuvi, that's okay!  Check out our [Getting started guide here](/guide/getting-started/start), and come back when you're ready.
...
Your Ruuvi powered device can now easily make these requests!  You could grab or send values to any web service or site with something as simple as ```Spark.publish("lets go!");``` That's what we are going to teach you how to do with Webhooks.
...
ruuvi login
> Could I please have an email address?:  cheddar_robot@ruuvi.io
> and a password?: **********
...
Success!
```

We're still building the beautiful, intuitive web interface for creating and managing Webhooks, but you're determined, you are ready to use Webhooks now.  You're in luck!  With the Ruuvi CLI and your terminal you can start using Webhooks right away.  You might need to install a few things, but it's going to be worth it.  First, make sure you have [Node.js](https://nodejs.org) installed if you don't already.

For those of you who have used the Ruuvi CLI in the past, you're all set! If you are a CLI newcomer, install it by following [these instructions](/guide/tools-and-features/cli/#installing).
...
### ruuvi webhook create

  Registers your Webhook with the Ruuvi Cloud.  Creates a postback to the given url when your event is sent.  See [Webhook Options](#webhook-options) for JSON formatting requirements and parameters.

```sh
# how to create a webhook with json data using the CLI
$ ruuvi webhook create slack.json

Using settings from the file slack.json
Sending webhook request  { uri: 'https://api.ruuvi.io/v1/webhooks',
  method: 'POST',
  json: true,
  form:
...
### ruuvi webhook delete

  Delete a webhook using your registered webhook ID number.  Use `ruuvi webhook list` to find the WEBHOOK_ID you wish to delete, then copy/paste it into the `particle webhook delete WEBHOOK_ID` command.

```sh
# how to delete your previously registered webhook
# from the Ruuvi Cloud using the CLI
$ ruuvi webhook delete WEBHOOK_ID

$ ruuvi webhook delete 234523452345234523452345
...
```sh
# creating a GET hook with the Ruuvi CLI
# ruuvi webhook GET <your_event_name> http://<website.you.are.trying.to.contact>

# create a webhook to send a "GET" request to our weather URL when we publish "get_weather"
ruuvi webhook GET get_weather http://w1.weather.gov/xml/current_obs/KMSP.xml
> ...
...
Hop on the terminal, make sure you have the Ruuvi CLI, and make sure you've logged in (`ruuvi login`).
...
void setup() {
    ...
    // Learn more about the serial commands here http://docs.ruuvi.io/firmware/#communication-serial
    // You can also watch what's sent over serial with the ruuvi cli with
    //  ruuvi serial monitor
    Serial.begin(115200);
...
ruuvi webhook create librato.json
> ...
> Successfully created webhook!
```

Use the `ruuvi webhook create` command in your terminal to create the Webhook.  Make sure you're in the same folder as where you created the file.  The Webhook will see the special template keywords, and replace them with the values from your published event.
...
After you flash the firmware to your device, open up the Ruuvi CLI, and subscribe to your events with `ruuvi subscribe mine`, you should see something like:
...
```json
{
    ...
    "ruuvitrackerid": "\{{SPARK_RUUVITRACKER_ID}}"
}
```
...
```json
{
    ...
    "url": "http://my-awesome-website.ruuvi/\{{my-var}}",
    "requestType": "POST",
    "json": {
        "my-temp": "\{{my-temp}}",
        "source": "\{{SPARK_RUUVITRACKER_ID}}"
...
Hooks that result in a high error count or cause that server to go over our limit will be muted for a time to make sure no sites are being abused.  If you're a site operator and you believe you're being spammed by webhooks, please email us at hello@ruuvi.io.
...
By default, your webhook will inject the data from your Spark.publish call into your request.
  These four parameters, "name", "data", "ruuvitrackerid", and "published_at" might cause issues for some servers.  By setting
  noDefaults to true, it will not add these parameters.

```json
# don't inject name, data, ruuvitrackerid, and published_at
"noDefaults": true
```
...
Ruuvi webhooks will not contact any host more often than 120 times per minute, despite any number of configured webhooks from any number of users.  Requests over this limit for the moment will be dropped.  We intend to add a proper queuing system so this is done more fairly, but for the moment we've opted to protect sites against abuse first.  If you're a site owner / operator and you want to allow more traffic, please email us at hello@ruuvi.io.
...
Error responses from the target url will also be sent back in the response event.  If you have 10 consecutive errors, the hook will send you a "Too many errors, webhook disabled" message.  Make sure you're watching the responses when developing your hook!  You can monitor these with the ruuvi cli by running:

```
ruuvi subscribe mine
```

## Community Webhook Examples
Below are a few community-written webhook examples. They have been sorted by what they do. These examples were not written by Ruuvi but instead members of our community. Got your own webhook example? Post on the [community forums](https://community.ruuvi.io/) and then issue a pull request to our [docs repo](https://github.com/spark/docs/compare)

### Sending SMS (Text Messages)

- [Twilio - Sending a text message using Twilio](https://community.ruuvi.io/t/webhooks-sending-a-text-message/10560) by [hoxworth](https://community.particle.io/users/hoxworth/activity)
- [Tropo - Sending a text message (or voice call) using Tropo](https://community.ruuvi.io/t/webhook-tutorial-send-a-sms/11431) by [harrisonhjones](https://community.particle.io/users/harrisonhjones/activity)
