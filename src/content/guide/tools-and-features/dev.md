...
**Ruuvi Dev** is a desktop application that allows you to work with local copies of your firmware files. However, **internet** access is required as the files are pushed to the Particle Cloud for compilation and returns a binary. i.e. This is not an offline development tool, yet.

All the commands are available from the **Ruuvi** menu. The menu changes depending on whether you're logged in or have selected a device, so some of the commands will only show up once you're in the right context.
...
There's also a toolbar on left side of IDE which contains shortcuts to the most frequently used commands like compiling and flashing (looks a lot like the one from [Web IDE (Build)](https://build.ruuvi.io/), doesn't it?).

### Logging In

If you want to work on more advanced projects, Ruuvi Dev could be the choice for you. Head over and download latest release:

[Ruuvi Dev Download >](https://www.ruuvi.io/dev)

![IDE Window]({{assets}}/images/ide-window.jpg)

To access most of features you need to log in using your Ruuvi account (which you can [create here](https://build.ruuvi.io/signup)) by clicking the link on the bottom of the window.
...
**NOTE**: When using [Command Line](/guide/tools-and-features/cli) you'll notice that log-in status is shared between Ruuvi Dev and CLI. So if you successfully ran `ruuvi login`, you will be logged in within the Particle Dev.

### Selecting Device

![Selecting device]({{assets}}/images/ide-select-ruuvitracker.jpg)
...
Different devices usually require separate binaries (i.e. you can't flash RuuviTracker with firmware compiled for a RuuviTag) and resulting file is going to be compiled for platform of currently selected device.  If you don't have a device selected, the code is going to be compiled for the Core.

**NOTE**: Remember that **\*.cpp** and **\*.ino** files behave differently. You can read more about it on our [support page](http://support.ruuvi.io/hc/en-us/articles/204952620).
...
To access all registered variables and functions, go to **Ruuvi** -> **Show cloud functions**/**Show cloud functions** menus.
...
When you want to check variable value constantly, you can click **Watch** button. When a variable is watched, Ruuvi Dev will fetch latest value every 5 seconds.
...
If your device isn't in {{#if ruuvitag}}{{{popup 'Listening Mode,' 'vine' 'https://vine.co/v/eZUH7WaWjMT/embed/simple'}}}{{/if}}{{#if ruuvitracker}}{{{popup 'Listening Mode,' 'vine' 'https://vine.co/v/eZU6YiK20Hl/embed/simple'}}}{{/if}} you'll see animation showing how to enter that state.
...
Find the [library you want to use](/guide/getting-started/build/ruuvitag/#using-libraries)
