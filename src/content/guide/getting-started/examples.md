...
Here you will find a bunch of examples to get you started with your new Ruuvi device! The diagrams here show the RuuviTag, but these examples will work with either the Photon or the RuuviTracker.
...
* **Hardware**
  ...
  * (2) Resistors between 220 Ohms and 1000 Ohms {{#if ruuvitag}}(220 Ohm Resistors included with RuuviTag Kit and Maker Kit){{/if}}
  * (1) LED, any color {{#if ruuvitag}}(Red LED included with RuuviTag Kit and Maker Kit){{/if}}
  * (1) Photoresistor {{#if ruuvitag}}(Included with RuuviTag Kit and Maker Kit){{/if}}
* **Software**
  * A text editor such as [Sublime](http://www.sublimetext.com/) or [TextMate](https://macromates.com/)
  * The [online IDE](http://build.ruuvi.io) or [Ruuvi Dev](http://particle.io/dev)
* **Experience**
...
![One LED illustration](/assets/images/ruuvitag-led-fritzing.png)

Next, we're going to load code onto your ruuvitracker. Copy and paste this code into a new application on http://build.ruuvi.io or on Ruuvi Dev. We've heavily commented this code so that you can see what is going on in each line.

Go ahead and save this application, then flash it to your RuuviTracker or RuuviTag. You should be able to see that LED blinking away!
...
Every program based on Wiring (programming language used by Arduino, and Ruuvi devices) has two essential parts:
...
It blinks the D7 LED on your Ruuvi device. If you have an LED wired to D0, it will blink that LED as well.
...
// This one is the little blue LED on your board. On the RuuviTag it is next to D7, and on the RuuviTracker it is next to the USB jack.
...
<html>
  ...
  <br>
  <br>
  <form action="https://api.ruuvi.io/v1/devices/your-device-ID-goes-here/led?access_token=your-access-token-goes-here" method="POST">
...
`ruuvi call device_name led on`

and

`ruuvi call device_name led off`
...
# RuuviTracker ID is 0123456789abcdef
# Your access token is 123412341234
curl https://api.ruuvi.io/v1/devices/0123456789abcdef/led \
...
![Fritzing Diagram](/assets/images/ruuvitag-photoresistor-fritzing.png)
...
Copy and paste the following code into your [online IDE](http://build.ruuvi.io) or [Ruuvi Dev](http://particle.io/dev) environment.
...
Or we can use the Ruuvi CLI with the command:
...
# RuuviTracker ID is 0123456789abcdef
# Your access token is 123412341234
curl -G https://api.ruuvi.io/v1/devices/0123456789abcdef/analogvalue \
  -d access_token=123412341234
```

You can see a JSON output of your Spark.variable() call by going to:

https&#58;//api.ruuvi.io/v1/devices/your-device-ID-goes-here/analogvalue?access_token=your-access-token-goes-here
...
`ruuvi variable get device_name analogvalue`
...
You can check out the results on your dashboard at [dashboard.ruuvi.io](https://dashboard.particle.io). As you put your finger in front of the beam, you'll see an event appear that says the beam was broken. When you remove your finger, the event says that the beam is now intact.
...
// On the RuuviTracker, it's the LED in the upper right hand corner.
// On the RuuviTag, it's next to the D7 pin.
...
Go find a buddy who has a RuuviTracker or RuuviTag. Your buddy does not have to be next to you-- he or she can be across the world if you like! All you need is Wi-Fi.

Connect your device and copy the firmware on the right into a new app. Pick a weird name for your event. If your ruuvitracker were named `starfish` for example, you could make your event name something like `starfish_special_event_20150515`. Have your buddy pick a name for their event as well. No more than 63 ASCII characters, and no spaces!
...
If your device is new, it already has the Tinker firmware on it. It's the default firmware stored on your device right from the factory. When you put your own user firmware on your device, you'll rewrite the Tinker firmware. (That means that your device will no longer understand commands from the Ruuvi mobile app.) However, you can always get the Tinker firmware back on your device by putting it in [factory reset mode](/ruuvitag/modes/#selecting-various-modes-factory-reset), or by re-flashing your device with Tinker in the Particle app.
