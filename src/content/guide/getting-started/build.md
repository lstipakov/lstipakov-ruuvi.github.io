...
Flash Apps with Ruuvi Build
===

Logging In
---

![Ruuvi Build](/assets/images/ide-new-account.png)
...
If you've already logged into Ruuvi Build before, click the "Let me log in" text beneath the Sign Up button, and you'll be presented with a login for existing users.  Don't worry--if you already have an account and accidentally click the "Sign Up" button, we'll still log you into your existing account.


Web IDE
---

![Ruuvi Build](/assets/images/ide-main.png)

Ruuvi Build is an Integrated Development Environment, or IDE; that means that you can do software development in an easy-to-use application, which just so happens to run in your web browser.

Ruuvi Build starts with the navigation bar on the left. On the top, there are three buttons, which serve important functions:
...
- **Docs**: Brings you to the documentation for Ruuvi.
...
Ruuvi Apps and Libraries
---

![Ruuvi Build](/assets/images/ide-apps.png)

The heart of Ruuvi Build is the "Particle Apps" section, which displays the name of the current app in your editor, as well as a list of your other applications and community-supported example apps.
...
- **Delete**: Click the "Remove App" button to remove it forever from your Ruuvi library.

- **Rename**: You can rename your Ruuvi App by simply double-clicking on the title of your app under the "Current App" header.  You can modify the "Optional description" field in the same way.
- **My Apps**: Tired of working on your current project?  Select the name of another app under the "My apps" header to open it in a tab of the Ruuvi Build editor.
...
- **Connect**: Make sure your device is powered and "breathing" Cyan, which indicates that it's connected to the Ruuvi Cloud and ready to be updated.
- **Get Code**: Try clicking on the "Blink an LED" example under the "Example apps" header.  The Ruuvi Build editor should display the code for the example application in an active tab.  Alternatively, you can copy and paste this snippet of code into a new application in the Build IDE.
...
```
![Ruuvi Build](/assets/images/ide-devices.png)
...
![Ruuvi Build](/assets/images/ide-examples.png)
...
![Ruuvi Build](/assets/images/ide-add-files.png)
...
There are a couple of other neat bells and whistles in Ruuvi Build.  The Particle Build IDE the best tool for viewing important information about your device, managing devices associated with your Particle account, and "unclaiming" them so they can be transferred to your buddy.

![Ruuvi Build](/assets/images/ide-account.png)
...
- **Unclaim**: You can "Unclaim" a device by pressing the "Remove Device" button that is revealed by clicking the dropdown arrow.  Once a device has been unclaimed, it is available to be reassociated with any Ruuvi users' account.

![Ruuvi Build](/assets/images/ide-settings.png)

- **API Key**: You can find your most recent API Key listed under the "Settings" tab in your account.  You can press the "Reset Token" button to assign a new API Key to your account.  *Note* that pressing this button will require you to update any hard-coded API Credentials in your Ruuvi-powered projects!
...
When you want to reuse code across multiple applications, Ruuvi Libraries are your friend.
Ruuvi Libraries are easily shareable, extensible packages built by the community to help with common problems many Particle applications encounter. They are hosted on GitHub and easily pulled into the IDE where they can be included in apps and shared with others.
...
An *embedded system* like the RuuviTracker or RuuviTag doesn't have an Operating System like a traditional computer. Instead, it runs a single application, often called *firmware*, which runs whenever the system is powered.
...
In our case, because the RuuviTrackers and RuuviTags are connected to the internet, updating firmware is quite trivial; we send it over the network, and we have put in place safeguards to keep you from "bricking" your device.
...
In most cases you want to build against Ruuvi's latest system firmware (which is used by default). If you need to target an older version (i.e. when newer version has some breaking changes) you can select it in dropdown located in device details.
...
Ruuvi Build uses a local cache to improve its performance. In some cases this may cause errors or outdated information about libraries. If you encounter similar symptoms try clearing the cache by going to **Settings** and clicking **Clear cache** button.
