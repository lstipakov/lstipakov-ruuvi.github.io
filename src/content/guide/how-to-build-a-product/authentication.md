...
For any Ruuvi-powered product to function properly, there are **four involved
parties**:
...
Understanding how all four interact, manage data, and  secure information is critical to launching a successful product on the Ruuvi platform.
...
interact with Ruuvi to control devices and manage customers.
...
## OAuth
Ruuvi tightly adheres to OAuth specifications to ensure secure and
...
Ruuvi API to perform actions that only your organization's team
members or applications should be able to perform. An example of this is
creating a customer that belongs to your organization via the Ruuvi
...
the Ruuvi API will allow you to create **access tokens**, which are
...
[Ruuvi dashboard](https://dashboard.ruuvi.io/) and click
“Create app” to create a client ID and secret.
(Ping @zachary in the [forum](http://community.ruuvi.io/)
...
Ruuvi API on behalf of your organization and your customers to manage all the
...
the `/` in `dashboard.ruuvi.io/`. Example:
`dashboard.ruuvi.io/my-org`, where `my-org` is the organization slug.
...
## Access Tokens
A related concept to understand is how Ruuvi uses access
tokens for authentication and security. If you have ever logged into the
Web IDE, called a function on a Ruuvi device, or read a variable via
...
An *access token* is a special code that is tied to a Ruuvi customer
...
https://api.ruuvi.io/v1/devices/0123456789abcdef01234567/lightsOn \
-d arg=livingRoom
```
...
For security purposes, most access tokens will expire after 90 days. Certain access tokens can be specifically set to *never expire*, like the one you can find in the settings pane of `build.ruuvi.io`.
...
the desired calls to the Ruuvi API to successfully read data from and
...
Luckily, the mobile SDKs and RuuviJS will expose helper methods to
...
end-users of your product are referred to as *customers* in the Ruuvi
...
will likely be influenced by how much you would like to hide Ruuvi from your
...
There are three ways to manage authentication for your customers.
  * [**Simple Authentication**](#simple-authentication): Customers are created and managed directly by Ruuvi. This is the simplest and fastest way to get your connected product app working.
  * [**Two-legged Authentication**](#two-legged-authentication): You create and manage accounts for your customers yourself, and request scoped access to control customers' devices from Ruuvi. This provides the maximum amount of flexibility and security for your application.
  * [**Login with Ruuvi**](#login-with-ruuvi): Your customers will create a Particle account and a separate account on your website, and link the two together using OAuth 2.0 *(Coming soon)*.
...
the Ruuvi API directly for both session management and device
interactions. Below is a diagram communicating how simple authentication
works at a high level:

![Simple authentication with Ruuvi](/assets/images/simple-auth-high-level.png)
<p class="caption">Your application interacts directly with the Ruuvi
...
account. Behind the scenes, your mobile app hits the Ruuvi API
...
mobile app uses the Ruuvi API to successfully claim the device to the customer
...
All communiation flows from the mobile client to the Ruuvi cloud,
then down to the customer's device.

### Advantages of Simple Auth

Simple auth is ideal for getting a Ruuvi product up-and-running quickly. Without needing to build your own back-end, development time to
creating an app to work with a Ruuvi device is greatly reduced. There are less moving parts and opportunities to introduce bugs. In
addition, Ruuvi's [mobile SDKs](/reference/ios/) and [JavaScript
...
Ruuvi from your customers. The SDKs allow for [front-end skinning and
...
interaction with Ruuvi will happen behind the scenes, hidden from
...
the Ruuvi API. This is much less secure than server to server communication.
...
You will create your OAuth client using the Ruuvi dashboard *(Coming Soon)*.

![Creating OAuth Clients via the Ruuvi Dashboard](/assets/images/creating-oauth-clients.png)
<p class="caption">You can use the Ruuvi Dashboard to create OAuth clients <em>(Coming Soon)</em></p>
...
If you are building a **mobile app** for your Ruuvi product, the cURL command to
...
```
Breaking this down:
* The Authorization header includes *your Ruuvi user's* access token. This user must be a team member of the organization that you are creating the client for.
...
-d redirect_uri=http://www.ruuvi.io/setup -d organization=my-org \
-d scope=create_customer https://api.ruuvi.io/v1/clients
```
...
In order for a device to be setup successfully, your application must retrieve a claim code on behalf of the customer setting up their device and send that claim code to the device. When the device receives proper Wi-Fi credentials and is able to connect to the Internet, it sends the claim code to the Ruuvi cloud. The Particle cloud then links the device to the customer, and grants the customer access over that device.

The first thing that must happen is retreiving a claim code from the Ruuvi cloud for the customer. A special endpoint exists for organizations to use to generate claim codes on behalf of their customers. 
...
This happens by connecting the customer's device to the *device's Wi-Fi access point*. When the ruuvitag is in [listening mode](/guide/getting-started/modes/ruuvitracker/#listening-mode), it is broadcasting a Wi-Fi network that the customer's computer or phone can connect to.

Once the customer's device is connected to the Ruuvi device's network, your mobile app then will send the claim code generated in the last step to the Particle device.
...
Now that your app is connected directly to the customer's Ruuvi-powered device, it can provide the device with Wi-Fi credentials to allow it to connect to the Internet.
...
The device uses the Wi-Fi credentials to connect to the Internet, and immediately sends the claim code to the Ruuvi cloud. At that point, the device is considered "claimed" by the customer. This means that any access token generated by that customer can be used to read data from or interact with the device.
...
Your application, armed with the customer's access token, can now successfully authenticate with any device-specific Ruuvi API endpoint, giving it full access and control over the device. Some of the things you can do include:
* Call functions on the device
* Read variable values from the device
* See if the device is online or not
* Much more!

Note that now, your app will never communicate directly to the device. The customer will trigger a call to the Ruuvi API, which then communicates with the device to carry out the desired action.

#### Further Considerations

Signup and device claiming only will happen one time for each customer. After this has been completed, subsequent visits to your application will continue to use customer access tokens to interact with the device via the Ruuvi API.
...
The main difference between two-legged and simple authentication is the presence of a back-end architecture to compliment your mobile or web application. Your application would communicate with both your server as well as the Ruuvi cloud.
...
Two-legged authentication is the ideal choice for a product creator looking for maximum visibility, control, and flexibility of their web or mobile application. With two-legged, you gain the ability to implement custom logic, integrations with third-party services, and store application-specific data that are not currently part of the Ruuvi platform. 
...
Another advantage of two-legged authentication is beefed-up security. Server-to-server communication (your server to the Ruuvi API) is much more secure than client-to-server communication (your mobile/web application to the Particle API). For sensitive transactions like passing OAuth credentials to get customer access tokens, using your server to talk to the Particle API over HTTPS is safe and protected.
...
Like Simple Authentication, you will need to create valid OAuth client credentials for your organization. Unlike simple authentication, your OAuth client credentials will be sent to the Ruuvi API from your server, not directly from your mobile/web application. The client credentials will be used for two purposes:
...
For now, you'll need to create your client by hitting the Ruuvi API directly. Your cURL request should look something like this:

```bash
curl -X POST -H "Authorization: Bearer 1234" -d name=MyApp -d type=installed \
-d organization=my-org https://api.ruuvi.io/v1/clients
```
...
Your *server* will need access to your newly created OAuth client ID and secret. Unlike simple authentication, **both** client ID and secret will be needed for two-legged authentication. Your server should have access to the client credentials anytime it needs to make an API call to Ruuvi.
...
-d no_password=true https://api.ruuvi.io/v1/orgs/particle/customers
```
Note that there is no password for the customer. An email address is the only piece of information required to create a customer in the Ruuvi system, and **must be collected by your application during signup**. As a result,  you must pass the `no_password=true` flag to create the customer with no password. Note that in this endpoint, you should use your client ID and secret instead of an access token.
...
It is important to note that the device setup process will not involve your server. All communication will be between your web/mobile application, the customer's device, and the Ruuvi cloud. These steps will also be handled by the mobile & JavaScript SDKs, involving extra work from you.
...
Once the device has been setup, and the customer created, you're now ready to interact with the device! Hooray! It's important to understand that while you do have your own server, **we recommend hitting the Ruuvi API directly from your application client for any device-related actions**. This includes:
...
<p class="caption">For all direct interactions with the device, hit the Ruuvi API from your application client</p>

The alternative would be telling your application client hit your back-end server, which would then trigger a call to the Ruuvi API. This introduces an extra intermediary in communication, and involves needing to unnecessarily "wrap" Particle API endpoints.

The *only* reason for your server to hit the Ruuvi API is to generate new scoped access tokens for your customers. You should generate a fresh access token each time the customer logs into your application.
...
-d scope=customer=jane@example.com https://api.ruuvi.io/oauth/token
```
...
https://api.ruuvi.io/oauth/token
```
