---
title: RuuviTag datasheet
template: datasheet.hbs
columns: two
order: 4
---

# RuuviTag Datasheet <sup>(v011)</sup>

**Model number:** RUUVITAGH

<div align=center><img src="/assets/images/ruuvitag_vector2_600.png" width=200></div>
...
Ruuvi's Internet of Things hardware development kit, the RuuviTag, provides everything you need to build a connected product. Particle combines a powerful ARM Cortex M3 micro-controller with a Broadcom Wi-Fi chip in a tiny thumbnail-sized module called the PØ (P-zero).

To get you started quickly, Ruuvi adds a rock solid 3.3VDC SMPS power supply, RF and user interface components to the PØ on a small single-sided PCB called the RuuviTag. The design is open source, so when you're ready to integrate the Photon into your product, you can.

The RuuviTag comes in two physical forms: with headers and without. Prototyping is easy with headers as the Photon plugs directly into standard breadboards and perfboards, and may also be mounted with 0.1" pitch female headers on a PCB. To minimize space required, the Photon form factor without headers has castellated edges. These make it possible to surface mount the Photon directly onto your PCB.

### Features

* Ruuvi PØ Wi-Fi module
...
<div align=center><img src="/assets/images/ruuvitag-block-diagram.png" width=600></div>
...
The RuuviTag has ton of capability in a small footprint, with analog, digital and communication interfaces.
...
Pin D3 through D7 are JTAG interface pins.  These can be used to reprogram your RuuviTag bootloader or user firmware image with standard JTAG tools such as the ST-Link v2, J-Link, R-Link, OLIMEX ARM-USB-TINI-H, and also the FTDI-based Ruuvi JTAG Programmer.

| RuuviTag Pin | Description | STM32 Pin | PØ Pin # | PØ Pin Name | Default Internal<sup>[1]</sup> |
| :-:|:-:|:-:|:-:|:-:|:- |
...
<div align=center><img src="/assets/images/ruuvitag-jtag.png" width=700></div>

---

### External Coexistence Interface

The RuuviTag supports coexistence with Bluetooth and other external radios via the three gold pads on the top side of the PCB near pin A3.  These pads are 0.035" square, spaced 0.049" apart.  This spacing supports the possibility of tacking on a small 1.25mm - 1.27mm pitch 3-pin male header to make it somewhat easier to interface with.

<div align=center><img src="/assets/images/ruuvitag_bt_coex.png" width=400></div>
...
<div align=center><img src="/assets/images/ruuvitag_pin_markings_1400.png"></div>
...
| Pin | Description |
|-|-|
| VIN | This pin can be used as an input or output. As an input, supply 3.6 to 5.5VDC to power the RuuviTag. When the Photon is powered via the USB port, this pin will output a voltage of approximately 4.8VDC due to a reverse polarity protection series schottky diode between VUSB and VIN. When used as an output, the max load on VIN is 1A. |
...
<div align=left><img src="/assets/images/ruuvitag-pinout1.png"</div>

<div align=left><img src="/assets/images/ruuvitag-pinout2.png"</div>

<div align=left><img src="/assets/images/ruuvitag-pinout3.png"</div>
...
These specifications are based on the STM32F205RG datasheet, with reference to RuuviTag pin nomenclature.
...
The RuuviTag (with headers) can be mounted with (qty 2) 12-pin single row 0.1" female headers.  Typically these are 0.335" (8.5mm) tall, but you may pick a taller one if desired.  When you search for parts like these it can be difficult to navigate the thousands of parts available.
...
### Recommended pcb land pattern (RuuviTag with headers)

The RuuviTag (with headers) can be mounted with 0.1" 12-pin female header receptacles using the following PCB land pattern:

<div align=left><img src="/assets/images/ruuvitag_land_pattern_with_headers.png" width=300></div>

This land pattern can be found in the [Spark.lbr Eagle library](https://github.com/spark/ruuvitag/blob/master/libraries/Spark.lbr), as a Device named `RUUVITAG`.  **Note: Clone or Download the complete repository as a ZIP file to avoid corrupted data in Eagle files.**

### Recommended pcb land pattern (RuuviTag without headers)

The RuuviTag (without headers) can be surface mounted directly in an end application PCB using the following PCB land pattern:

<div align=left><img src="/assets/images/ruuvitag_land_pattern_without_headers.png" width=600></div>

RuuviTag Pin #25-31 are described in the [Pin Out Diagrams](#pin-out-diagrams).
...
This land pattern can be found in the [Spark.lbr Eagle library](https://github.com/spark/ruuvitag/blob/master/libraries/Spark.lbr), as a Device named `RUUVITAG_SMD`.  **Note: Clone or Download the complete repository as a ZIP file to avoid corrupted data in Eagle files.**

## Schematic

### Schematic - USB

<div align=center><img src="/assets/images/ruuvitag-sch-usb.png" width=400></div>

### Schematic - Power

<div align=center><img src="/assets/images/ruuvitag-sch-power.png" width=600></div>

### Schematic - User I/O

<div align=center><img src="/assets/images/ruuvitag-sch-user-io.png" width=600></div>

### Schematic - RF

<div align=center><img src="/assets/images/ruuvitag-sch-rf.png" width=500></div>

### Schematic - PØ Wi-Fi Module

<div align=center><img src="/assets/images/ruuvitag-sch-p0-wifi-module.png" width=500></div>

## Layout

### RuuviTag v1.0.0 Top Layer (GTL)
<div align=center><img src="/assets/images/ruuvitag_v100_pcb_top.png" width=200></div>

### RuuviTag v1.0.0 GND Layer (G2L)
<div align=center><img src="/assets/images/ruuvitag_v100_pcb_gnd.png" width=200></div>

### RuuviTag v1.0.0 3V3 Layer (G15L)
<div align=center><img src="/assets/images/ruuvitag_v100_pcb_3v3.png" width=200></div>

### RuuviTag v1.0.0 Bottom Layer (GBL)
<div align=center><img src="/assets/images/ruuvitag_v100_pcb_bot.png" width=200></div>

## Recommended solder reflow profile

<div align=left><img src="/assets/images/ruuvitag-reflow-profile.png" width=600></div>
...
### Build your own design based on the RuuviTag!
...
RuuviTags are available from [store.ruuvi.io](https://store.particle.io/) in single quantities with and without headers, and also included in different maker kits.
...
The RuuviTag comes in two primary styles of packaging: Matchbox and Kit Box.  The Matchbox contains the bare essentials to get you started, while the kit box contains a breadboard, Micro B USB cable, sticker, prototyping card and a couple sensors to build your first internet connected project!

![ruuvitag-packaging](https://lh3.googleusercontent.com/-E7B5bPuCIvU/VVOs6bdsAmI/AAAAAAAAA_c/5NQ5v6Rnhm0/s0/photon-packaging.png)

RuuviTags without headers are also available in JEDEC style trays for automated pick and place machines.  Request more details from us on this in the Contact section below.

### Moisture sensitivity levels

<i class="icon-attention"></i> The Moisture Sensitivity Level (MSL) relates to the packaging and handling precautions required. The PØ module on the RuuviTags dominate the MSL requirements and are rated level 3. In general, this precaution applies for Photons without headers.  If reflowing a Photon directly onto an application PCB, increased moisture levels prior to reflow can damage sensitive electronics on the Photon.  A bake process to reduce moisture may be required. <i class="icon-attention"></i>
...
<dt>Castellated</dt>
<dd>The edge of the PCB has plated holes that are cut in half which resemble the top of a castle.  These make it easy to solder the RuuviTag down to another PCB with a SMT reflow process.</dd>
<dt>SMT</dt>
...
> Contains FCC ID: 2AEMI-RUUVITAG
...
> Contains transmitter module IC: 20127-RUUVITAG
...
| v011 | 24-July-2015 | BW | Added FCC IC CE Warnings and End Product Labeling Requirements, Updated power output, added approved antennas, Corrected DAC2 as A3, Added pin numbers to PCB Land Pattern for RuuviTag without headers. |
...
https://community.ruuvi.io

**Email**

<mailto:hello@ruuvi.io>
