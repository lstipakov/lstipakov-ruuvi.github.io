---
title: RuuviTracker datasheet
template: datasheet.hbs
columns: two
order: 2
---

# Spark RuuviTracker Datasheet
...
![STM32](/assets/images/ruuvitracker-stm32.jpg)

Spark RuuviTracker v1.0 uses the STM32F103CB - ARM 32-bit Cortex M3 based - microcontroller for its brain power. You can download the datasheet [here.](https://github.com/spark/ruuvitracker/blob/master/Datasheets/ST_STM32F103CB.pdf)

Some of its key features are as follows:

- ARM 32-bit Cortex™-M3 CPU RuuviTracker
...
![CC3000](/assets/images/ruuvitracker-cc3000.jpg)

RuuviTracker v1.0 uses TI's CC3000 module for the WiFi communications.
...
This means, you can power the RuuviTracker via the USB port or via the VIN pin from an external power supply that can range from 3.6V to 6.0V DC. Ideal sources of power can be: 3.6V LiPo battery, 4AA battery pack, backup USB battery or an USB wall charger.

![Powering the RuuviTracker](/assets/images/ruuvitracker-power.jpg)

### RF circuit

![RF Circuit](/assets/images/ruuvitracker-rf.jpg)
...
You can download a copy of the RF test report [here.](/assets/images/ruuvitracker-rf-test-report.pdf)

## Pins and I/O

![Spark Pinout](/assets/images/spark-pinout.png)

The Spark RuuviTracker offers a total 18 I/O pins to the user: `D0 to D7`, `A0 to A7` and two pins that are preset to serial - `TX` and `RX`. All of these I/O pins run at *3.3V* and the user should keep this in mind before attaching any external peripherals to them. The only exception to this are the following pins that are tolerant to 5V inputs:
...
Each pin on the RuuviTracker can either be configured as input (with or without pull-up or pull-down) or as output (push-pull or open-drain) using the [pinMode()](/reference/firmware/#pinmode-) function.
...
![Hardware USART](/assets/images/ruuvitracker-pin-usart.jpg)

The RuuviTracker features three serial ports. The first one is a CDC (Communications Device Class) available over the USB port. When configured, it will show up as a virtual COM port on the computer. (usage: `Serial.begin(9600);`)

The second one is a hardware USART available via the TX and RX pins on the RuuviTracker. (usage: `Serial1.begin(9600);`)

The third one is a hardware USART available via the D1(Tx) and D0(Rx) pins on the RuuviTracker. (usage: `Serial2.begin(9600);`)

Configuration and use of all of these serial ports is described in the [serial functions.](/reference/firmware/#serial)

**NOTE:** Please take into account that the voltage levels on these pins runs at 0V to 3.3V and should not be connected directly to a computer's RS232 serial port which operates at +/- 12V and can damage the RuuviTracker.

### SPI

![SPI](/assets/images/ruuvitracker-pin-spi.jpg)
...
![I2C](/assets/images/ruuvitracker-pin-i2c.jpg)
...
![JTAG](/assets/images/ruuvitracker-pins-jtag.jpg)
...
The hardware files for the JTAG shield are available [here.](https://github.com/spark/shields/tree/master/ruuvitracker-shields/Programmer%20Shield)
...
<table border = '1'>
   ...
   <tr>
      <td>0x08005000</td>
      <td>RuuviTracker Firmware Location</td>
      <td>108 KB max</td>
   </tr>
</table>
...
![S11 Chart](/assets/images/ruuvitracker-s11-chart.png)
...
## Types of RuuviTrackers

![Antenna Options](/assets/images/chip-ufl.jpg)

Currently the RuuviTracker is available in two flavors. With an on-board chip antenna or an uFL connector to connect an external antenna.

### Chip Antenna


This version of the RuuviTracker comes with an on board chip antenna that gives a signal strength similar to a mobile phone.  In most cases, a simple plastic enclosure will not affect the signal strength of the chip antenna.

### uFL Connector

If you want to improve the signal strength of the RuuviTracker, you can connect an external antenna with the help of the [uFL connector](http://en.wikipedia.org/wiki/Hirose_U.FL). Most antennas that are designed to operate at 2.4GHz or are WiFi rated will do the job. You can also make your own [cantenna](https://community.ruuvi.io/t/extending-the-ruuvitracker-wifi-range-with-a-cantenna/1357)!


## Physical layout

![DIMENSIONS](/assets/images/ruuvitracker-dim.jpg)

The header pins on the RuuviTracker are spaced at an interval of 0.1", which is the standard pitch size for proto-boards and breadboards. The physical layout of the Core was inspired from the [Arduino Pro Mini](http://arduino.cc/en/Main/ArduinoBoardProMini) board.

Mechanical drawings of the RuuviTracker are available [here.](https://github.com/spark/ruuvitracker/blob/master/PDFs/ruuvitracker-mechanical-drawing-v1.pdf)
